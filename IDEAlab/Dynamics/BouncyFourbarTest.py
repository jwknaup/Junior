...
def createBlock(name, pos, rot, scale, mass, pcm):
    dx = scale[2]
    dy = scale[1]
    dz = scale[0]
    ...
    dictionary = {'pcm':pcm, 'vcm':vcm, 'y':y,'q':q, 'q_d':q_d, 'ini':initialvalues, 'dx':dx, dy':dy}
    
    return dictionary

def createFourbar(length):   
    x = Differentiable('x',system)
    y = Differentiable('y', system)
    top = x*N.x+y*N.y
    lt_cm = -length/2*cos(q)
    lt = createBlock('lt', [-length/2*cos(45),-length/2*sin(45),0], pi/4, [length,0.001,0.01], 0.01)
    lb = createBlock('lb', [-length/2*cos(45),-3*length/2*sin(45),0], 3*pi/4, [length,0.001,0.01], 0.01)
    rt = createBlock('lt', [length/2*cos(45),-length/2*sin(45),0], -pi/4, [length,0.001,0.01], 0.01)
    rb = createBlock('lb', [length/2*cos(45),-3*length/2*sin(45),0], -3*pi/4, [length,0.001,0.01], 0.01)
