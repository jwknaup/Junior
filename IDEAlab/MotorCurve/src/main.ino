...
void setup() {
  ...
  //point to the ISRs
  setupISRs();
  //initialize 0th Joint now that pin numbers have been assigned
  initializeJoint(0);

  analogWrite(Joint[0].enPort,255);
  delay(500);
  analogWrite(Joint[0].enPort,0);
...
void encAngleUpdate(short jointNum){
  const unsigned int countsPerEdgePerChannel = 3, numChannels = 2, numEdges = 1, gearRatio = 1000;
...
float velocityGet(int time){
  const unsigned int countsPerEdgePerChannel = 3, numChannels = 2, numEdges = 1, gearRatio = 298;
  const unsigned long countsPerRev = countsPerEdgePerChannel*numChannels*numEdges*gearRatio;
  float perMin = (float)time/60000;
  static long ticksLast=0, delta;
  delta = Joint[0].ticks - ticksLast;
  ticksLast = Joint[0].ticks;
  return (float)delta/countsPerRev/perMin;
}

void loop() {

  analogWrite(Joint[0].enPort,255);
  Serial.println(velocityGet(1000));
  ...
  // Serial.println(Joint[0].force);
  //Serial.println(Joint[0].target);
  // Serial.println(Joint[0].error);

  //forceVsForce();
  //maintainForce(.03, 0, -1150);
